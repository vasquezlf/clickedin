...
router.delete("/:id", passport.authenticate("jwt", { session: false }), (req, res) => {
  ...
  // .then(profile => {
      Post.findById(req.params.id)
      .then(post => {
        // Check for post owner | post.user comes in as an object so need to convert it to string
...
router.post("/like/:id", passport.authenticate("jwt", { session: false }), (req, res) => {
  ...
  .then(profile => {
      Post.findById(req.params.id)
      .then(post => {
        // Go thru array of likes inside the Post, then see if any of likes id
        // matches with current logged in user(req.user.id). User already liked it
